...
function Notification() {
  ...
  useDocumentTitle(title || "Notification Management");
  // -- PAGE LOAD END --
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const response = await requestApi.post(
        "/general/setup/notifications/mass-delete",
        {
  ...
  const filteredData = useMemo(() => {
    const source = data;
    return (
      source?.filter((item: NotificationData) => {
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-red-700 text-sm leading-relaxed">
                  The selected notification records will be permanently removed
                  from the system.
                </p>
              </div>
            </div>
